...
public class Indexer {
    ...
    public Indexer(File indexDir1Sa, File indexDir1Cat, File indexDir2) throws IOException {
        ...
        //many DBpedia types are equivalent to wikidata e.g. http://www.wikidata.org/entity/Q6256 = Country apod. -> musim to zaindexovat temi DBpedia typy a ne wikidata
        String dbpediaFile = "ontology.owl";
	        	...
	        	for (OntClass cl1 : cl.listEquivalentClasses().toList()) {
	        		if(cl1.toString().matches("http://www.wikidata.org/entity/.*")||cl1.toString().matches("http://schema.org/.*")) {
		        		if(wikiToDBclasses.containsKey(cl1.getURI().toString()))
		        			...
		        			wikiToDBclasses.put(cl1.getURI().toString(), new ArrayList<>(Arrays.asList(cl.getURI().toString())));
	        		}
	        	}
        }
    }
    ...
    public Tuple[] search1Sa(String resource, int top_n) {
    	try {
            //final ScoreDoc[] hits = this.is1Sa.search(new TermQuery(new Term("subject", resource)), 1000).scoreDocs;
