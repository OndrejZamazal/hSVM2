...
public class HierarchicalModelGeneration {
	...
	//create ontology only containing DBpedia classes for later propagation
	OntModel m = ModelFactory.createOntologyModel( OntModelSpec.OWL_MEM );
	HashSet<String> uniqueClasses;
	//final String sparqlService="http://dbpedia.org/sparql";
	...
	public HierarchicalModelGeneration(boolean indexLucene, String dbpediaFile, String lang_short_abstracts, String lang_article_categories, String lang_instance_types) {
		try {
	        ...
	        if(indexLucene) {
	        	InputStream is1 = new GZIPInputStream(new FileInputStream(lang_short_abstracts));
	        	InputStream is2 = new GZIPInputStream(new FileInputStream(lang_article_categories));
	        	InputStream is3 = new GZIPInputStream(new FileInputStream(lang_instance_types));
	...
	private void random_entities_pick_lucene(PrintWriter toFile1, PrintWriter toFile2, String dbpediaClass, int max) {
		try {			    		    	
			...
			//System.out.println(listOfResources.length);
	...
	private void traverse_classes_hierarchy1(boolean createOntology, PrintWriter toFile, OntClass cls, Integer level, int min_instances) {
		...
		if(instances>=min_instances) {
			...
			if(createOntology) {
				...
				com.hp.hpl.jena.rdf.model.Statement statement = ResourceFactory.createStatement(s, prop, o);
			...
			//System.out.println(level+":"+cls.getLocalName()+":"+cls.listSubClasses(true).toList().size()+":"+instances);
    		...
    		if(createOntology) {
    			for(OntClass clsSup : cls.listSuperClasses(true).toList()) {
					...
					com.hp.hpl.jena.rdf.model.Statement statement = ResourceFactory.createStatement(s, prop, oR);
				    this.m.add(statement);
				}
			}
	...
	public void traverse_classes_hierarchy_start(boolean createOntology, int min_instances) {
		try {			
			...
			//System.out.println(this.uniqueClasses.size());
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	public String get_categories_and_short_abstracts_for_resource_lucene(String resource) {
	    ...
	    //System.out.println("shortAbstractEn:"+shortAbstractEn);
	    return cats+":"+shortAbstractEn;
	}
	
	public void generate_arff_files(String svmModelsFile) {
		try {
			...
			//vstup = new BufferedReader(new FileReader ("res/svmModels.txt"));
			vstup = new BufferedReader(new FileReader ("res/"+svmModelsFile));
			String s="";
			StringTokenizer st = null;			
			while ((s = vstup.readLine()) != null) {
				//31-03-15, there was exception due to comments with # at the beginning
			...
			for(int i=0;i<SVM_models.size();i++) {
				//System.out.println(this.SVM_models.get(i).size()+":"+this.SVM_models.get(i));
				...
				//toFile2.println("@attribute className {Organisation,Food,Device,Place,CelestialBody,SportsSeason,Event,ChemicalSubstance,Drug,Award,Biomolecule,TimePeriod,TopicalConcept,Language,AnatomicalStructure,Holiday,MeanOfTransportation,Person,Currency,Species,Work,EthnicGroup,Disease,SportCompetitionResult,UnitOfWork,Activity,Name,Database,Colour,SnookerWorldRanking}");
				...
				for(String className : SVM_models.get(i)) {
					//get data
					random_entities_pick_lucene(toFile1,toFile2,className,100);
				}
				toFile1.flush();
				toFile1.close();
				toFile2.flush();
	...
	public void adjustOntology(String ontology) {
		try {	
			...
			for (OntClass cls : ont.listClasses().toList()) {
				...
				//if(n>0&&n<3) { //min__number_subclasses=3
				if(n>0&&n<5) { //min__number_subclasses=5
					//System.out.println(cls.listSuperClasses(true).toList());
	...
	public void init_ontology_cache(String ontology) {
		try {
			...
			//superTypes=all superclasses having some subclasses
	...
	public void subclassesSplitIntoSVMmodels(String ontology) {
		try {
			...
			OntModel ont = ModelFactory.createOntologyModel(OntModelSpec.OWL_MEM );
			//ont.read("file:///home/ondrej/eclipse-maven/res/"+ontology, "RDF/XML" );
			ont.read("res/"+ontology, "RDF/XML" );
			...
			//toFile.println(SVM_model.size()+":"+SVM_model);
			sb.append(SVM_model.size()+":"+SVM_model+"\n");
			//toFile.flush();
	...
	public void filter_arff_files(String svm_models) {		
		try {
			...
			//BufferedReader swFile = new BufferedReader(new FileReader("res/stopwords.en.txt"));
				...
				if(line.indexOf("-")!=-1) {
					...
					while(true) {
						...
						//for(String s : atts.keySet()) {
							//toFile.print(" "+s+":"+unique_attributes_sa.indexOf(s.trim())+":"+atts.get(s).freq);
							//toFile.print(" "+unique_attributes_sa.indexOf(s.trim())+":"+atts.get(s).freq);
